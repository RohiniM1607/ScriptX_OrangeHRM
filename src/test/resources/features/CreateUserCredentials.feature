...
Feature: Rohini_14May2026_OrangeHRM_Create User Credential
  ...
  Scenario Outline: Create user with duplicate username
    ...
    Examples:
      ...
      | Employee | Employee 1   | Enabled | Admin    | Employee1@123 | Employee@123    |

  @Rohini @CreateCredential @Password_Mismatch
  Scenario Outline: Create user with password mismatch
    When user clicks on Add button
    And user enters user credential details with password mismatch "<role>" "<employeeName>" "<status>" "<username>" "<password>" "<confirmPassword>"
    And user clicks on Save button
